   ...
   socket.on('newUser', (user) => {
    ...
    socket.on('createCart', async () => {
        try {
            ...
            const idCart = await cart._id
            console.log(idCart)
            socketServer.emit('newCart', idCart)
        } catch (error) {
   ...
   //LISTA DE PRODUCTOS DISPONIBLES PARA CARRITOS-----------------------------------
    socket.on('getListproducts', async () => {
        try {
            ...
            socketServer.emit('listProducts1', productsArray)
        } catch (error) {
            console.error('Error al agregar el producto:', error.message);
        }
    })
    ...
    socket.on('addProduct1', async (addProduct) => {
        try {
            const cid = addProduct.idC
            const id = addProduct.idP
            const product = await cartService.addProdToCart(cid,id);
    ...
    //LIMPIAR CARRITO---------------------------------------------------------
    socket.on('cleanCart', async (idCart) => {
        try {
    ...
    socket.on('deleteCart', async (idCart) => {
        try {
            const cart = await cartService.deleteCart(idCart);
            let x = 0
            if (cart) { x = "" }
            console.log ("el nuevo x es", x)
            socketServer.emit('cart does not exist', x)
        } catch (error) {
            console.error('Error al borrar el carrito:', error.message);
        }
    })

    /* El problema radica en cómo se asigna el valor a la variable x. El código actual siempre asigna 0 a x y luego intenta reasignarlo dentro del bloque if, pero esta reasignación no se refleja fuera del bloque if debido al uso del const.
    Para corregir esto, debes usar let en lugar de const para que la reasignación de x sea válida. 
    */
