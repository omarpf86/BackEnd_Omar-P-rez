...
const ticketDao= new TicketDaoMongo()
        ...
        export const generateTicket = async (user) => {
            try {
            console.log("en el service el ticket es", user)
            console.log("en el service el id de car es", user.cid)
            ...
            if (cart.products.length > 0) {
                for (const x of cart.products) {
                    ...
                    console.log("en ticket service el id del producto comprado es",idProd)
                    const prodDB = await productService.getById(idProd);
                    console.log("en ticket service el producto es", prodDB)

                    if (x.quantity <= prodDB.stock) {
                        const amount = x.quantity * prodDB.price;
                        console.log("en ticket service la cantodad es",amount)
                        amountAcc += amount;
                    } else return null;
                }
            }
